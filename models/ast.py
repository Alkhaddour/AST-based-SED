...
class ASTModel(nn.Module):
    ...
    def __init__(self, label_dim=527, fstride=10, tstride=10, input_fdim=128, input_tdim=1024, imagenet_pretrain=True,
        ...
        cprint('---------------AST Model Summary---------------', verbose=verbose)
        cprint('ImageNet pretraining: {:s}, AudioSet pretraining: {:s}'.format(str(imagenet_pretrain),
                                                                               str(audioset_pretrain)), verbose=verbose)
        ...
        if audioset_pretrain == False:
            ...
            cprint('frequncey stride={:d}, time stride={:d}'.format(fstride, tstride), verbose=verbose)
            cprint('number of patches={:d}'.format(num_patches), verbose=verbose)
            ...
            if imagenet_pretrain == True:
                ...
                new_pos_embed = self.v.pos_embed[:, 2:, :].detach().\
                    reshape(1, self.original_num_patches, self.original_embedding_dim).transpose(1,2).\
        ...
        # now load a model that is pretrained on both ImageNet and AudioSet
            ...
            if audioset_pretrain == True and imagenet_pretrain == False:
                raise ValueError('currently model pretrained on only audioset is not supported, please set '
                                 'imagenet_pretrain = True to use audioset pretrained model.')
...
#     # print(summary(ast_mdl, (input_tdim, 128)))
#     cprint(test_input, False)
#
